...
esp_err_t create_DB_users(void)
{
    ...
    ESP_LOGW(AUTH_TAG, "path DB : %s", USERS_FILEPATH);
...
esp_err_t create_user(char *email, char *password, char *azz_id)
{
    ...
    if (!fp)
    {
        fclose(fp);
        cJSON_Delete(_users);
        ESP_LOGE(AUTH_TAG, "Failed to create user");
        return ESP_FAIL;
    }
    fputs(users_data, fp);
    fclose(fp);
...
esp_err_t check_user(char *email, char *password)
{
    ...
    if (admins_array_size > 0)
    {
        for (size_t i = 0; i < admins_array_size; i++)
        {
            ...
            if (strstr(email, _email) && strstr(password, _pass))
            {
                ESP_LOGW(AUTH_TAG, "admin : %s", email);
    ...
    if (users_array_size > 0)
    {
        for (size_t i = 0; i < users_array_size; i++)
        {
            ...
            if (strstr(email, _email) && strstr(password, _pass))
            {
                ESP_LOGW(AUTH_TAG, "user : %s", email);
                // httpd_resp_sendstr(req, _email);
                cJSON_Delete(_users);
                return ESP_OK;
            }
...
esp_err_t check_DB_users(void)
{
    ...
    FILE *fp = fopen(USERS_FILEPATH, "r");
    if (!fp)
    {
        ESP_LOGW(AUTH_TAG, "not DB users");
        fclose(fp);
        return create_DB_users();
    }
    fclose(fp);
    return ESP_OK;
}
...
esp_err_t auth(char *buf)
{
    ...
    if (check_DB_users() == ESP_FAIL)
    {
        ESP_LOGW(AUTH_TAG, "error check DB users");
        return ESP_FAIL;
    }
    ...
    if (check_user(email, password) == ESP_FAIL)
    {
        return ESP_FAIL;
    }
