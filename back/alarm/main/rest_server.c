...
// static user_t active_users[MAX_USERS];
// static int num_active_users = 0;


// #define KEY_LENGTH 32

// char *generate_access_key() {
...
// esp_err_t check_file_users(void)
// {
// 	// Проверка наличия файла
// 	FILE *file = fopen("/spiffs/users.json", "r");
// 	if (file == NULL)
// 	{
// 		ESP_LOGI(REST_TAG, "Файл /spiffs/users.json не найден. Создание файла...");

// 		// Создание объекта JSON для списка пользователей
// 		cJSON *users_array = cJSON_CreateArray();
// 		cJSON *user = cJSON_CreateObject();
// 		cJSON_AddStringToObject(user, "username", "goose");
// 		cJSON_AddStringToObject(user, "password", "goose");
// 		cJSON_AddBoolToObject(user, "is_admin", true);
// 		cJSON_AddItemToArray(users_array, user);

// 		// Преобразование объекта JSON в строку и запись в файл
// 		char *json_str = cJSON_PrintUnformatted(users_array);
// 		ESP_LOGW(REST_TAG, "users_array : %s", users_array);
// 		file = fopen("/spiffs/users.json", "w");
// 		if (file == NULL)
// 		{
// 			ESP_LOGE(REST_TAG, "Ошибка открытия файла для записи");
// 			return;
// 		}
// 		fwrite(json_str, strlen(json_str), 1, file);
// 		fclose(file);

// 		ESP_LOGI(REST_TAG, "Файл /spiffs/users.json успешно создан");
...
// 	{
// 		fclose(file);
// 		ESP_LOGI(REST_TAG, "Файл /spiffs/users.json найден");
// 	}
// 	return ESP_OK;
// }

// int get_access_token(void) {
//     ESP_LOGI(REST_TAG, "get access token : %d", access_token);
//     return access_token;
// }

// void set_access_token() {
//     ESP_LOGI(REST_TAG, "before set access token : %d", access_token);
//     access_token++;
//     ESP_LOGI(REST_TAG, "after set access token : %d", access_token);
// }
...
//         set_access_token();
//         char resp_data[256] = {0};
//         sprintf(resp_data, "{\"admin\":\"true\",\"access_key\":\"%d\",\"ok\":\"true\"}", get_access_token());
...
static esp_err_t rest_audio_list_get_handler(httpd_req_t *req) {
    ...
    if (dir == NULL) {
        ...
        cJSON_AddStringToObject(root, "music_list", "");
        const char *sys_info = cJSON_Print(root);
        httpd_resp_sendstr(req, sys_info);
        free((void *)sys_info);
        cJSON_Delete(root);
...
static esp_err_t rest_calls_list_get_handler(httpd_req_t *req) {
    ...
    const char *filepath = "/spiffs/calls.json";
    int fsize(char* file) {
        int size;
        FILE* fh;
...
static esp_err_t erase_wifi_handler(httpd_req_t *req) {
    ESP_LOGI("[ ERASE WIFI ]", "Erasing Wi-Fi credentials and rebooting...");
    httpd_resp_sendstr(req, "Erase WiFi");
    esp_wifi_disconnect();
	esp_wifi_stop();
	esp_wifi_deinit();
	nvs_flash_erase();
...
esp_err_t start_rest_server(const char *base_path) {
    ...
    /* Устанавливаем базовый путь spiffs */
    REST_CHECK(base_path, "wrong base path", err);
    rest_server_context_t *rest_context = calloc(1, sizeof(rest_server_context_t));
    REST_CHECK(rest_context, "No memory for rest context", err);
    strlcpy(rest_context->base_path, base_path, sizeof(rest_context->base_path));

    /* Устанавливаем базовый путь sdcard */
    ...
    /* Обработчик URI для получения громкости */
    ...
    /* Обработчик URI для получения списка мелодий с SD карты */
    ...
    /* Обработчик URI для получения списка звонков */
    ...
    httpd_uri_t erase_wifi_uri = {
        .uri = "/erase_wifi",
        .method = HTTP_POST,
        .handler = erase_wifi_handler,
        .user_ctx = rest_context
    };
    httpd_register_uri_handler(server, &erase_wifi_uri); 
    ...
    /* Обработчик URI для установки громкости */
    ...
    /* Обработчик URI для установки списка звонков */
    httpd_uri_t set_calls_post_uri = {
        .uri = "/set_notifications",
        .method = HTTP_POST,
    ...
    /* Обработчик URI для загрузки мелодий на сервер */
    ...
    /* Обработчик URI для получения файлов веб-сервера */
