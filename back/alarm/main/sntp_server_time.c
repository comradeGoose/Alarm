...
static const char *TAG_WIFI_ERASE = "[ WIFI_ERASE ]";
#define BUTTON_GPIO_PIN		GPIO_NUM_19   // Пин, к которому подключена кнопка

static char path_ad_sd[128] = {0};
static char azz_check_time[64] = {0};

// static audio_board_handle_t audio_board_handle;
...
void check_calls(void)
{
	...
	if (fp == NULL)
	{
		ESP_LOGE(TAG_SNTP, "Failed to open file calls.json");
...
void general_view(int azz_hour, int azz_min)
{
	if (azz_hour > 9)
	{
		if (azz_min > 9)
...
char *azz_check(int azz_w_day, int azz_hour, int azz_min)
{
	...
	for (int i = 0; i < size_bell_list; i++)
	{
		...
		cJSON *week_days = cJSON_GetObjectItem(selecred_item, "daysOfTheWeek");
		char *w_days = cJSON_Print(week_days);

		if (strstr(w_days, time_wday) == NULL)
		{
			ESP_LOGI(TAG_SNTP, "continue -->  days Of The Week: %s", w_days);
			ESP_LOGI(TAG_SNTP, "---------------------------------------------");
			continue;
		}
		// free((void *)time_wday);
		ESP_LOGI(TAG_SNTP, "days Of The Week %d: %s", i, w_days);

		char *time_start = cJSON_GetObjectItem(selecred_item, "startLesson")->valuestring;
		ESP_LOGI(TAG_SNTP, "start Lesson: %s", time_start);
		char *time_end = cJSON_GetObjectItem(selecred_item, "endLesson")->valuestring;
		ESP_LOGI(TAG_SNTP, "end Lesson: %s", time_end);

		if (strstr(time_start, azz_check_time) == NULL &&
				strstr(time_end, azz_check_time) == NULL)
		{

			ESP_LOGI(TAG_SNTP, "continue --> azz_check_time: %s,start Lesson: %s, end Lesson: %s", azz_check_time, time_start, time_end);
			ESP_LOGI(TAG_SNTP, "---------------------------------------------");
			continue;
		}
		...
		const char *song_name = cJSON_GetObjectItem(selecred_item, "music")->valuestring;

		if (strncmp(song_name, "", 1) == 0)
		{
			ESP_LOGW(TAG_SNTP, "Song name not found, using default bell.mp3");
			memset(path_ad_sd, 0, sizeof(path_ad_sd));
			sprintf(path_ad_sd, "spiffs://spiffs/bell.mp3");
			ESP_LOGI(TAG_SNTP, "__AUDIO_PATH__ : %s ", path_ad_sd);
			cJSON_Delete(root);
			return &path_ad_sd;
		}

		ESP_LOGI(TAG_SNTP, "Timely PLAY -- %s", song_name);
		ESP_LOGI(TAG_SNTP, "---------------------------------------------");

		memset(path_ad_sd, 0, sizeof(path_ad_sd));
		sprintf(path_ad_sd, "file://sdcard/%s", song_name);
...
static void initialize_sntp(void)
{
	...
	sntp_setservername(0, NTP_SERVER); // "pool.ntp.org"
		...
		if (st.status != AUDIO_STATUS_RUNNING)
		{
			...
			else
			{
				if (dt.second == 0)
				{
					...
					{
						ESP_LOGI(TAG_SNTP, "NO CALL");
					}
					else
					{
						if (strncmp(audio_path, "file", 4) == 0)
						{
							...
							if (esp_player_sdcard_music_play(path_ad_sd, 0) == ESP_ERR_AUDIO_OPEN)
							{
...
void button_task(void* arg)
{
    ...
    while (1) {
        ...
        if (erase_wifi) {
			ESP_LOGI(TAG_WIFI_ERASE, "Erasing Wi-Fi credentials and rebooting...");
            esp_wifi_disconnect();
			esp_wifi_stop();
			esp_wifi_deinit();
			nvs_flash_erase();
...
static void init_app(void)
{
	...
	ESP_LOGI(TAG_SNTP, "[Step 9.0 ] : BUTTON WIFI ERASE");
